...
def predict_post():
    if request.method == 'POST':
        ...
        df_clicks=pd.read_csv("input/clicks2.csv", index_col=0)
        nbusers=df_clicks["user_id"].nunique()
        nbarticles=df_clicks["click_article_id"].nunique()
        ...
        #Déclaration de l'URL pour atteindre la fonction AZURE
        
        url="https://fonction-serverless.azurewebsites.net/api/HttpTriggerServerless"
        code="R6D6DokTlrQnn8AuA6v6Atf3ZQ33UvY61nGtHOucc-teAzFuIOYZCw=="
        var_params={'userID':userID}
        response=requests.get(url, params=var_params, headers={"x-functions-key":code})
        
        if  response.status_code==200:
            
            return render_template('recommendations.html', recoms=response.text, userID=userID,nbusers=nbusers,nbarticles=nbarticles)
        else :
            return "L'appel de la fonction Azure est NOK"
